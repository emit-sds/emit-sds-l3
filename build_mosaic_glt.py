...
def main():
    ...
    if args.glt_file_list is not None:
        ...
        glt_files = np.squeeze(np.array(pd.read_csv(args.glt_file_list,header=None)))
    ...
    if args.igm_file_list is not None:
        ...
        igm_files = np.squeeze(np.array(pd.read_csv(args.igm_file_list,header=None)))
    ...
    if args.criteria_file_list is not None:
        ...
        criteria_files = np.squeeze(np.array(pd.read_csv(args.criteria_file_list,header=None)))
    ...
    for igm_ind, igm_file in enumerate(igm_files):
        ...
        igm_dataset = gdal.Open(igm_file,gdal.GA_ReadOnly)
        #igm = np.transpose(np.memmap(igm_file, mode='r', shape=(igm_dataset.RasterYSize, igm_dataset.RasterCount, igm_dataset.RasterXSize), dtype=np.float32).copy(),(1,0,2))
        igm = igm_dataset.ReadAsArray()
    ...
    if args.stop_line is None:
        args.start_line = y_size_px 

    if args.start_line > y_size_px:
        logging.error('start line: {} exceeds image # of lines: {}'.format(args.start_line, y_size_px))
        quit()

    if args.stop_line > y_size_px:
        logging.info('end line: {} down adjusted to image # of lines: {}'.format(args.stop_line, y_size_px))
        args.stop_line = y_size_px
...
def construct_mosaic_glt_line(output_file: str, glt_files: np.array, criteria_files: np.array, size_px: tuple,
    ...
    for file_index, (glt_file, px_offset, lr_px_coord) in enumerate(zip(glt_files, px_offsets, lr_px_coords)):
        ...
        glt = np.transpose(np.memmap(glt_file, mode='r', shape=(glt_dataset.RasterYSize, glt_dataset.RasterCount, glt_dataset.RasterXSize), dtype=np.int16)[line_index-px_offset[1]:line_index-px_offset[1]+1,:,:].copy(),(1,0,2))
